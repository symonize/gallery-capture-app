import {
  json,
  requireAuth,
  airtableBase,
  airtableHeaders,
  AIRTABLE_API,
} from "./_shared.js";

const TABLE = process.env.ARTISTS_TABLE || "Artists";

/*
  GET  -> list all artists [{ id, name }]
  POST -> create artist { name, bio?, nationality? } -> { id, name }
*/
export async function handler(event) {
  const auth = requireAuth(event);
  if (auth) return auth;

  const base = airtableBase();

  if (event.httpMethod === "GET") {
    const records = [];
    let offset;
    try {
      do {
        const url = new URL(`${AIRTABLE_API}/${base}/${encodeURIComponent(TABLE)}`);
        url.searchParams.set("pageSize", "100");
        if (offset) url.searchParams.set("offset", offset);
        const res = await fetch(url, { headers: airtableHeaders() });
        if (!res.ok) return json(res.status, { error: `Airtable ${res.status}: ${await res.text()}` });
        const data = await res.json();
        records.push(...data.records);
        offset = data.offset;
      } while (offset);
    } catch (e) {
      return json(502, { error: `Airtable request failed: ${e.message}` });
    }
    return json(200, records.map((r) => ({ id: r.id, name: r.fields.name || "" })));
  }

  if (event.httpMethod === "POST") {
    let payload;
    try {
      payload = JSON.parse(event.body || "{}");
    } catch {
      return json(400, { error: "Invalid JSON body." });
    }
    const { name, bio = "", nationality = "" } = payload;
    if (!name?.trim()) return json(400, { error: "Missing name." });

    const res = await fetch(`${AIRTABLE_API}/${base}/${encodeURIComponent(TABLE)}`, {
      method: "POST",
      headers: airtableHeaders(),
      body: JSON.stringify({ fields: { name, bio, nationality }, typecast: true }),
    });
    if (!res.ok) return json(res.status, { error: `Airtable ${res.status}: ${await res.text()}` });
    const data = await res.json();
    return json(200, { id: data.id, name: data.fields.name });
  }

  return json(405, { error: "GET or POST only." });
}
